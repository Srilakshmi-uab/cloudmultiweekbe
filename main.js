...
app.post('/api/upload', upload.single('file') , async (req, res) => {
 try {
  ...
  let randomId = new Date().getDate().toString(36)+new Date().getTime().toString(36)
    ...
    console.log('==========')
    insertItemsToTable('file_upload_list', {id:randomId, emails:userEmails.join(','), file_name:req.file.originalname, uploaded_date:new Date(), file_url: JSON.parse(response.body).fileUrl})
    insertItemsToTable('file_clicks', {id:randomId, emails:userEmails.join(','),  file_url: JSON.parse(response.body).fileUrl, clicks: 0})
    const snsClient = new SNSClient({
      region: "us-east-2", 
      credentials: {
     accessKeyId: "AKIAUPCKTHUGWACQ2HXD",
    ...
    const message = 'Your file is ready to download:'+ `http://localhost:4200/fetch/id=${randomId}_url${fileUrl.split(".com")[1].replace(/^\/+/, '')}`;
